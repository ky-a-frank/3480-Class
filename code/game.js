...
var wobblyspeed = 1, wobblydist = 5;
Bunny.prototype.act = function(step) {
  this.wobble += step * wobblyspeed;
  var wobblePos = Math.sin(this.wobble) * wobblydist;
  this.pos = this.basePos.plus(new Vector(wobblePos, 0.35));
};
var wobbledspeed = 0, woddleddist = 0;
Drop.prototype.act = function(step) {
  this.pos = this.basePos.plus(new Vector(0, 0.5));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
  ...
  } else if (type == "bunny" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1.5;    
    }else if (type == "coin") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
      //coin++;
      //document.getElementById('coin').innerHTML = 'Food Eaten: ' + coin;
    });
    }else if (type == "drop"){ 
this.actors = this.actors.filter(function(other) {
      return other != actor;})
  ...
  function startLevel(n) {
    ...
    gravity = 20;
      if (status == "lost")
        ...
        console.log("You win! But did you collect all the food?");
    });
  }
  startLevel(0);
}
